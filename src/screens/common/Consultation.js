...
export default () => {
  ...
  const stringDateFormated = !state.consultationDateTime
    ? '__/___/'
    : moment(new Date(state.consultationDateTime)).format('DD/MMM');
  const stringTimeFormated = !state.consultationDateTime
    ? '   :  '
    : moment(new Date(state.consultationDateTime)).format('HH:mm');
      ...
      {!newConsultation && !user.employeeType === 'Vet' ? (
        <View style={styles.deleteButton}>
          <IconButton
            ...
            size={25}
            onPress={() => {
              if (state.idPet && state.idPet !== '') deleteConsultation();
            }}
          />
        </View>
      ) : null}
